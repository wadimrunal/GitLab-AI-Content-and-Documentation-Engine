...
class ContentJobCreateRequest(BaseModel):
    ...
    None,
    description="Selected GitLab project ID"
)
...
class ContentJobResponse(BaseModel):
    ...
    # NEW: lets the writer dashboard show "sent back for revision" drafts
    # separately from brand-new drafts, even though both currently sit in
    # the "drafting" status. True only when a reviewer/doc lead has sent
    # this job back with a "request_revision" action.
...
class AdminLogEntry(BaseModel):
    """One line from backend/audit.log, shown on the admin Logs screen."""

    timestamp: str
    event_type: str
    job_id: Optional[str] = None

    # Every other field logged with the event (created_by, reviewer,
    # action, new_status, exported_by, etc.) is free-form, so it's
    # collected here instead of being hard-coded field by field.
    model_config = ConfigDict(extra="allow")


class AdminUserResponse(BaseModel):
    """A registered account, for the admin Users screen. Never includes
    password_hash / password_salt — those never leave accounts.py."""
    model_config = ConfigDict(from_attributes=True)
